...
Status: accepted (merged on the owner's instruction to finish the work without a Vercel build check, which no assistant can do; see Consequences)
Date: 2026-09-20
...
- Node: `engines.node` is `>=22.12 <25` and is the single source of truth: Vercel reads only that field (resolves to 24.x today; Node 20 is disabled there on 2026-10-01), CI reads it through `setup-node` `node-version-file: package.json`, and `<25` keeps out Node 25, which Vitest 5 does not support; a future Node 26 with 24 dropped from Vercel would make the build fail loudly instead of moving silently. `mise.toml` (`node = "24"`) is for people who use mise and must stay inside the range. This departs from T49 (recorded in `.claude/tastes.md`).

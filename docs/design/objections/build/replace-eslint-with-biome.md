...
HISTORY:    r1 open (design-adversary) -> r1 proposed (integrator: suspicious/noEmptyBlockStatements is enabled as warn (one hit on main, an intentional `() => {}` console silencer in a test); no-namespace, no-require-imports and no-unused-expressions are listed in ADR 0004 as dropped and accepted: the repository is ESM TypeScript with no namespaces or require calls)
            ...
            -> r3d verified (design-adversary review pass r2: a planted `var` fails the lint (rc 1); a probe with @ts-nocheck, a namespace, require, a bare expression, a useless assignment, a catch-and-rethrow without cause, `"a" as "a"` and a @ts-expect-error without description triggered none of the lost rules (only noUnusedVariables and useNodejsImportProtocol fired); the ADR's list matches the round-1 comparison exactly)

[OBJ-5] candidate: plan | severity: minor | status: verified
...
HISTORY:    r1 open (design-adversary) -> r1 proposed (integrator: `!.claude/worktrees` added to files.includes (criterion 5 checks it); the warning budget (35 warnings + 1 info on main, 50 + 2 on the owner's tree) is recorded in ADR 0004; `--error-on-warnings` is left for after the noNonNullAssertion warnings are cleaned up)
            ...
            -> r3d verified (design-adversary review pass r2: `npm run lint` prints 'Diagnostics not shown: 17', 'Found 36 warnings.', 'Found 1 info.'; the breakdown (21 noNonNullAssertion, 10 useOptionalChain, 2 useImportType, 2 useIterableCallbackReturn, 1 noEmptyBlockStatements) sums to 36 and matches the ADR; `!.claude/worktrees` is in biome.json)

[OBJ-6] candidate: plan | severity: major | status: verified
...
HISTORY:    r2 open (design-adversary review pass r1) -> r3 proposed (integrator: ADR 0004 gets a paragraph "The owner's uncommitted tree" stating exactly that (their copy of the file is unmodified, so bringing main into their tree brings the fix); the criterion is reworded to require both measurements (with and without the fixed file); measured 2026-09-20: without 1 error / 50 warnings / 2 infos, with 0 errors / 50 warnings / 2 infos; hand-off tells the owner)
            -> r3d verified (design-adversary review pass r2: on a read-only copy of the owner's tree with a real node_modules copy: without the fixed dev-polling.ts 'Found 1 error, 50 warnings, 2 infos', with it 0 errors, 50 warnings, 2 infos; the ADR paragraph 'The owner's uncommitted tree' and the reworded criterion are accurate; not verified: that the pre-commit hook rejects on this error (it calls `npm run lint`, not re-read this round))

...
HISTORY:    r1 open (design-adversary)
            ...
            -> r3 note: the split into two pull requests recommended by the arbiter was REVERSED after the fresh-eyes review (see OBJ-17): the class fix (commit 2) and the tests in their final state (commit 3) ship in ONE pull request, so no commit or file carries a passing test that pins the unfixed behaviour; no test, no commit message and no source file on the branch contains the characterisation ('KNOWN DEFECT') rows or it.fails (this log and the decision record still discuss them as history)
...
[OBJ-17] candidate: A | severity: major | status: verified
...
HISTORY:    r3 open (design-adversary review pass r1: fresh eyes, confirmed by the ledger as N3) -> r3 proposed (integrator: the decision record's split is reversed, see its Amendment: branch rebuilt so that the order is build(deps), fix(scrape), test, ci, docs; the tests exist only in their final state (the address table refuses the fourteen spellings), so no commit and no file contains a passing 'KNOWN DEFECT' test; criterion 22 is dropped for this branch and criteria 36-39 apply to it; nothing was pushed before this change)
            -> r4d verified (design-adversary review pass r2: `git log --oneline origin/main..HEAD` shows build(deps), fix(scrape), test, ci, docs, docs in that order; `git log -p` over src, scripts and vitest.config.ts finds no 'KNOWN DEFECT', no it.fails and no skip/only/todo; the test commit's tree on top of the fix commit runs 6 files / 114 tests green; criterion 36 passes (62 assertions, the six named spellings are 'refuses' rows); caveat: the prose in decision-record.md and this log still mentions the earlier plan (R10, criterion 19, an implementation note), covered by the Amendment)

[OBJ-18] candidate: A | severity: major | status: verified
...
HISTORY:    r3 open (design-adversary review pass r1: fresh eyes) -> r3 proposed (integrator: a fake fetch that behaves like the real one (asked to follow it lands on the final page, otherwise it hands back the 302) makes the option observable through behaviour: 'never lets fetch follow a redirect on its own'; 'gives up on a redirect chain longer than the cap' (a chain that would end after six hops must return null) and 'follows a short chain of public redirects to the page'; rows for 198.19.255.255 and 100.127.255.255 (refused) and 198.17.255.255, 198.20.0.1 (allowed); scripts/mutation-battery.sh scrape-ipv4 now has the four mutants and kills 8/8, the ssrf class kills 10/10 including the code as it was on origin/main; suite 114 tests)
            -> r4d open (design-adversary review pass r2: the four named mutants are killed (1 failed / 113 passed each; battery scrape-ipv4 8/8, ssrf 10/10), but deleting the line `redirect: "manual",` survives all 114 tests, and so does `redirect: "error"`, because the fake fetch treats a missing option as manual while real Node follows by default (a local server run: {} -> followed, manual -> 302, follow -> followed); MAX_REDIRECTS 3->2 and 3->4 also survive, harmless because the re-check still runs on every hop, accepted)
            -> r4 proposed (integrator: the fake now follows unless it is told "manual", the exact default of real fetch; the battery gains the delete-the-line mutant; on copies the delete-line and the "error" variants are killed; scrape-ipv4 9/9, ssrf 10/10, 36 mutants in total; the MAX_REDIRECTS value stays unpinned by design)
            -> r5d verified (design-adversary review pass r3: `env -u DATABASE_URL -u BOT_TOKEN npm test` rc 0 (6 files, 114 tests); on a copy each of these mutants of src/lib/scrape.ts turns the suite red (rc 1, 1 failed): delete the `redirect: "manual",` line, `redirect: "error"`, `redirect: "follow"`, `redirect: undefined`, failing test 'never lets fetch follow a redirect on its own' (scrape.test.ts:121-127); `bash scripts/mutation-battery.sh scrape-ipv4` 9/9 and `ssrf` 10/10 exit 0; the fake follows unless told 'manual', an honest model of real fetch (undici defaults to follow), and no other test in the file passes only because of that treatment; not blocking, unverified: one test is the only guard for the option, and real undici was not run against a redirecting server)

[OBJ-19] candidate: all | severity: major | status: verified
...
HISTORY:    r3 open (design-adversary review pass r1: ledger, fresh eyes) -> r3 proposed (integrator: every quotation of the construct in docs/design is reworded so that no `.skip call` / `.only call` is followed by a quote or backtick; the guard is re-run end to end on the final range and its output is cited in docs/verification)
            -> r4d verified (design-adversary review pass r2: `KIT_BASE_REF=origin/main bash scripts/kit-ci-guards.sh` on HEAD printed 0 finding(s) (no skips, thresholds not lowered, no suppression, CI edited and flagged [ci-change]); scripts/kit-ci-guards.sh is not in the diff so the base-branch copy CI runs is identical; main-protection check ok; unverified: behaviour under GitHub's fetch-depth: 0 checkout)

[OBJ-20] candidate: A | severity: minor | status: accepted-risk
...
HISTORY:    r3 open (design-adversary review pass r1: fresh eyes) -> r3 proposed (integrator: the job declares `environment: staging`, and the workflow header says why a github.ref check written in this file would NOT be a boundary (the branch can edit the file) and that the restriction is the owner's to configure: create the `staging` Environment with deployment branches limited to main and store STAGING_DATABASE_URL there; CONTRIBUTING.md says so; accepted-risk until then: any writer can dispatch a branch and read the secret; bound: the workflow fails at its first step until the secret exists at all)
            -> r4d accepted-risk (design-adversary review pass r2: flows.yml:37 has `environment: staging` and the header (lines 15-22) says a github.ref check inside the file would not be a boundary and that the owner must create the Environment limited to main and store the secret there; GitHub documents that an unprotected Environment is auto-created, so the mechanism removes nothing until configured; bound: the first step fails until STAGING_DATABASE_URL exists, so no secret is exposed today (criteria 31 and 32 pass); not verified: whether repository-level secrets stay visible to environment jobs)

[OBJ-21] candidate: all | severity: minor | status: verified
...
HISTORY:    r3 open (design-adversary review pass r1: ledger) -> r3 proposed (integrator: regenerated with `npm install --package-lock-only --ignore-scripts` and folded into the dependency commit)
            -> r4d verified (design-adversary review pass r2: package-lock.json:33 reads >=22.12; `npm install --package-lock-only --ignore-scripts` on a copy leaves an empty diff; `npm ci --dry-run` exits 0)

[OBJ-22] candidate: A | severity: note | status: accepted-risk
...
HISTORY:    r3 open (design-adversary review pass r1: ledger) -> r3 proposed (integrator: accepted-risk: both are GitHub-owned actions and ci.yml uses the same tags; bound: the job is manual-only and cannot run until the secret exists; pinning both workflows to SHAs is a separate change)
            -> r4d accepted-risk (design-adversary review pass r2: flows.yml:46,48 uses actions/checkout@v4 and actions/setup-node@v4 as ci.yml:22,24 does; both are GitHub-owned actions so the rule's 'third-party' arguably does not apply; the job is manual-only and cannot run until the secret exists)

[OBJ-23] candidate: A | severity: note | status: verified
CLAIM:      The fix is not strictly "toward more blocking": the old text pattern refused four public spellings by accident which the numeric comparison no longer refuses.
EVIDENCE:   design-adversary review pass r2: on origin/main scrape.ts `[fd::1]`, `[fc::1]`, `[fc0::1]` and `[fdf::1]` were refused (hit=false on the fetch), on HEAD they connect; they are groups 0x00fc-0x00fd and 0x0fc0-0x0fdf, outside fc00::/7 and not private.
SCENARIO:   A reader trusts the record's "every changed line moves toward more blocking".
BAR:        The claim is corrected where it was made.
HISTORY:    r4 open (design-adversary review pass r2) -> r4 proposed (integrator: criterion 38 in ADR 0003 and the decision record's Amendment now say so; no security consequence)
            -> r5d verified (design-adversary review pass r3: docs/decisions/0003-unit-tests-and-ci-wiring.md criterion 38 names the four groups ([fd::1], [fc::1], [fc0::1], [fdf::1]) as the exception with no security consequence, and docs/design/decision-record.md's Amendment carries the same correction; the four-spelling reproduction was not re-run (round 2 did))
            -> r6 note (acceptance audit): a differential of the old and new isBlockedAddress (65,536 first-group values over 6 tails plus a 400,000-address random fuzz) measured the relaxed class as 34 first-group values (fc*, fd*, fc0-fcf, fd0-fdf), all outside fc00::/7; nothing else was relaxed and 260 addresses were tightened; criterion 38 and the Amendment now describe the class, not four examples
